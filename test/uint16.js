...
var smallArray = createArray(Uint16Array, Math.pow(2, 16) - 1, 100);
var largeArray = createArray(Uint16Array, Math.pow(2, 16) - 1, 45 * 1024);
var massiveArray = createArray(Uint16Array, Math.pow(2, 16) - 1, 1035 * 1024);


test('uint16: create test connections', function(t) {
  t.plan(2);
...
test('uint16: have dc connectivity', function(t) {
  t.plan(1);
...
test('uint16: create buffered channels for existing channels', function(t) {
  t.plan(2);
...
test('uint16: small array is chunked', checkChunks(smallArray, 1, 'uint16'));
test('uint16: small array is sent ok', function(t) {
  t.plan(1);
...
test('uint16: large array is chunked', checkChunks(largeArray, 6, 'uint16'));
test('uint16: large array is sent ok', function(t) {
  t.plan(1);
...
test('uint16: massive array is chunked', checkChunks(massiveArray, 130, 'uint16'));
test('uint16: massive array is sent ok', function(t) {
  t.plan(1);
