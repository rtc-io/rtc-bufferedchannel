...
test('uint8: create test connections', function(t) {
  t.plan(2);
...
test('uint8: have dc connectivity', function(t) {
  t.plan(1);
...
test('uint8: create buffered channels for existing channels', function(t) {
  t.plan(2);
...
test('uint8: small array is chunked', checkChunks(smallArray, 1, 'uint8'));
test('uint8: small array is sent ok', function(t) {
  t.plan(1);
...
test('uint8: large array is chunked', checkChunks(largeArray, 3, 'uint8'));
test('uint8: large array is sent ok', function(t) {
  t.plan(1);
...
test('uint8: massive array is chunked', checkChunks(massiveArray, 65, 'uint8'));
test('uint8: massive array is sent ok', function(t) {
  t.plan(1);
